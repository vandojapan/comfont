...
- 本文に半角の`<`が1文字でも含まれる場合、安全のため本文全体を加工せず返します。AviUtl2の
  制御文字、ルビ、スクリプトなどを含む本文には合成フォントが適用されません。全角の`＜`は使えます。
...
- `api_version()` — 現在のスクリプトモジュールAPIバージョン`9`を取得
...
> この節は未実装のロードマップです。現在利用できる機能や、beta版で動作を保証する仕様では
> ありません。SDK側の仕様が確定したときに詳細を調整します。
...
現行版は本文に半角の`<`があると、既存の制御文字を壊さないため本文をそのまま返します。将来は
AviUtl2の制御文字をトークン化し、その状態と合成フォントの補正を合成できる段階で、制御文字を
含む本文への適用を追加します。
